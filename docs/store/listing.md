...
TickTick Tasks puts the task list you already use on your Garmin watch. Pair your TickTick account once, then browse Today, Inbox, Overdue, Lists, and the tasks inside each list. Complete a task with a separate confirmation, or refresh when you need the latest state.
...
- Keep Today, Inbox, Overdue, and Lists visible together on wide watches, with a three-item band on mid-size screens; tap, swipe, or use the watch buttons.
